...
struct integration_test_t : Test {
  ...
  auto resolve() -> decltype(auto) {
    return static_cast<typename expected_t<resolved_t>::result_t>(
        sut.template resolve<resolved_t>());
  }

  template <typename value_t>
  auto test_resolve() {
    const auto& value = resolve<value_t>();
    const auto& ref = resolve<value_t&>();
    ASSERT_NE(&value, &ref);
  }
};
...
TEST_F(integration_test_t, refs_are_shared) {
  ASSERT_EQ(resolve<constructible_t<>&>().id,
            resolve<const constructible_t<>&>().id);
}

TEST_F(integration_test_t, rrefs_are_transient) {
  ASSERT_NE(resolve<constructible_t<>&&>().id,
            resolve<const constructible_t<>&&>().id);
}
...
TEST_F(integration_test_t, constructible_from_cref) {
  test_resolve<constructible_t<const constructible_t<>&>>();
}
...
TEST_F(integration_test_t, constructible_from_lref_rcref) {
  test_resolve<
      constructible_t<constructible_t<>&, const constructible_t<>&&>>();
}
...
TEST_F(integration_test_t, constructible_from_cref_val) {
  test_resolve<constructible_t<const constructible_t<>&, constructible_t<>>>();
}
...
TEST_F(integration_test_t, constructible_from_cref_rref) {
  test_resolve<
      constructible_t<const constructible_t<>&, constructible_t<>&&>>();
}

TEST_F(integration_test_t, constructible_from_cref_cref) {
  test_resolve<
      constructible_t<const constructible_t<>&, const constructible_t<>&>>();
}
...
struct integration_test_deep_graph_t : integration_test_t {
  template <typename value_t>
  using nested_t = constructible_t<value_t, value_t&, value_t&&, const value_t,
...
TEST_F(integration_test_deep_graph_t, resolve) { test_resolve<graph_t>(); }
