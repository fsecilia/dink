...
struct fixture_t
{
    ...
    using initial_type_list_t = type_list_t<int, void*>;
    ...
    struct resolver_t
    {
        ...
        template <typename deduced_t, typename dependency_chain_t>
    ...
    StrictMock<resolver_t> resolver{};

    auto expect_id() noexcept -> void { EXPECT_CALL(resolver, resolve_id()).WillOnce(Return(expected_id)); }
    auto expect_val() noexcept -> void { EXPECT_CALL(resolver, resolve_val()).WillOnce(Return(deduced)); }
    auto expect_ref() noexcept -> void { EXPECT_CALL(resolver, resolve_ref()).WillOnce(ReturnRef(deduced)); }
    auto expect_ptr() noexcept -> void { EXPECT_CALL(resolver, resolve_ptr()).WillOnce(Return(deduced_ptr)); }

    auto expect_ptr_ref() noexcept -> void
    {
        EXPECT_CALL(resolver, resolve_ptr_ref()).WillOnce(ReturnRef(deduced_ptr));
    }

    auto expect_cptr_ref() noexcept -> void
    {
        EXPECT_CALL(resolver, resolve_cptr_ref()).WillOnce(ReturnRef(deduced_cptr));
    }

    auto expect_arr_ref() noexcept -> void
    {
        EXPECT_CALL(resolver, resolve_arr_ref()).WillOnce(ReturnRef(deduced_array));
    }
};
...
struct arg_test_t : fixture_t, Test
{
    ...
    sut_t sut = dispatcher_t{}.create_sut(fixture_t::resolver);
...
{
    using sut_t = arg_t<fixture_t::resolver_t, fixture_t::initial_type_list_t>;
    auto create_sut(fixture_t::resolver_t& resolver) const noexcept -> sut_t { return sut_t{resolver}; }
};

struct single_arg_test_dispatcher_t
{
    using arg_t = arg_t<fixture_t::resolver_t, fixture_t::initial_type_list_t>;
    using sut_t = single_arg_t<fixture_t::handler_t, arg_t>;
    auto create_sut(fixture_t::resolver_t& resolver) const noexcept -> sut_t { return sut_t{arg_t{resolver}}; }
};
...
/*
    ...
    directly calling its single-argument ctor with arg_t or single_arg_t. Their behaviors are very different.
*/
...
{
    using sut_t = arg_t<fixture_t::resolver_t, initial_type_list_t>;
    sut_t sut = sut_t{resolver};

    // arg_t matches deduced_t's move ctor first, copy ctor second, id ctor third, but this is not useful in di
    expect_val();
...
{
    using arg_t = arg_t<fixture_t::resolver_t, initial_type_list_t>;
    using sut_t = single_arg_t<deduced_t, arg_t>;
    sut_t sut = sut_t{arg_t{resolver}};
