...
#include "append_only_heap_allocator.hpp"
#include <dink/test.hpp>

namespace dink {
namespace {

struct append_only_heap_allocator_test_t : Test
{
    ...
    std::size_t const expected_size_2 = 813;
    std::size_t const expected_size_3 = 20;
...
struct append_only_heap_allocator_test_default_container_t : append_only_heap_allocator_test_t
{
    using sut_t = append_only_heap_allocator_t<heap_allocator_t>;
    sut_t sut{heap_allocator_t{&mock_heap_allocator}};
};

TEST_F(append_only_heap_allocator_test_default_container_t, allocate_tracks_pointers)
{
    expect_all_allocate();
...
struct append_only_heap_allocator_test_throw_on_emplace_back_vector_t : append_only_heap_allocator_test_t
{
    struct throw_on_emplace_back_vector_t
    {
        auto begin() const -> void** { return nullptr; }
        auto end() const -> void** { return nullptr; }
        auto emplace_back(void*) -> void* { throw std::bad_alloc{}; }
    };

    using sut_t = append_only_heap_allocator_t<heap_allocator_t, throw_on_emplace_back_vector_t>;
    sut_t sut{heap_allocator_t{&mock_heap_allocator}, throw_on_emplace_back_vector_t{}};
};

TEST_F(append_only_heap_allocator_test_throw_on_emplace_back_vector_t, allocate_cleans_up_and_rethrows_on_bad_alloc)
{
    EXPECT_CALL(mock_heap_allocator, allocate(expected_size_1, expected_alignment_1))
        .WillOnce(Return(expected_allocation_1));
    EXPECT_CALL(mock_heap_allocator, deallocate(expected_allocation_1));
