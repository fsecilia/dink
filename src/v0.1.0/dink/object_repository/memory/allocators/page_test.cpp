...
struct page_reservation_test_t : Test
{
    ...
    using sut_t = reservation_t<allocator_t>;
    sut_t sut{allocator, allocation_begin, allocation_end};
};

TEST_F(page_reservation_test_t, allocation)
{
    ASSERT_EQ(allocation_begin, sut.allocation());
}

TEST_F(page_reservation_test_t, commit)
{
...
struct page_allocator_test_t : Test
{
    ...
    struct reservation_t
    {
        ...
        reservation_t(allocator_t& allocator, void* allocation_begin, void* allocation_end) noexcept
...
TEST_F(page_allocator_test_t, reserve_sets_page_field_in_reservation)
{
...
TEST_F(page_allocator_test_t, reserve_returns_nonempty_allocation_when_size_is_zero)
{
    ...
    ASSERT_LT(reservation.allocation_begin, reservation.allocation_end);
}
...
TEST_F(page_allocator_test_t, reserve_succeeds_when_size_exactly_fits_region)
{
    ...
    auto reservation = sut.reserve(size, std::align_val_t{1});
...
TEST_F(page_allocator_test_t, reserve_returns_nullptr_when_size_doesnt_fit_at_end_of_region)
{
    ...
    ASSERT_EQ(nullptr, reservation.allocation_begin);
    ASSERT_EQ(nullptr, reservation.allocation_end);
}
...
TEST_F(page_allocator_test_t, reserve_returns_nullptr_when_alignment_doesnt_fit_at_end_of_region)
{
    ...
    auto reservation = sut.reserve(1, align_val);
