...
struct allocation_caster_test_t : Test
{
...
TEST_F(allocation_caster_test_t, handles_nullptr)
{
    auto const result = cast_allocation<simple_t, dst_deleter_t<simple_t>>(
...
TEST_F(allocation_caster_test_t, constructs_object_and_transfers_deleter)
{
    auto const result = cast_allocation<simple_t, dst_deleter_t<simple_t>>(
...
TEST_F(allocation_caster_test_t, forwards_ctor_args)
{
    auto const expected_int = composite_t::default_int + 1;
    auto const expected_string = std::string{"expected"};

    auto result = cast_allocation<composite_t, dst_deleter_t<composite_t>>(
...
TEST_F(allocation_caster_test_t, handles_throw_from_ctor)
{
    EXPECT_CALL(mock_deleter, call(expected_allocation));

    EXPECT_THROW(
        (void)(cast_allocation<throwing_t, dst_deleter_t<throwing_t>>(
            allocation_t{expected_allocation, allocation_deleter_t{&mock_deleter}}
        )),
        std::runtime_error
    );
