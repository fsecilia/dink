...
class page_pending_allocation_t
{
public:
    auto allocation() const noexcept -> void* { return allocation_begin_; }
    auto commit() noexcept -> void { page_->commit(allocation_end_); }

    page_pending_allocation_t(page_t& page, void* allocation_begin, void* allocation_end) noexcept
...
{
public:
    using pending_allocation_t = pending_allocation_p<page_t>;
    ...
    page_t(void* begin, std::size_t size, std::size_t max_allocation_size) noexcept
